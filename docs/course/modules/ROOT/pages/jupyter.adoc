...
On a Linux host with Xvfb installed, you can then visualize the result using the `pyvista` package as follows. Install the bundle's `feelpp` extra with `uv sync --locked --extra feelpp` in the Python environment that provides the {feelpp} bindings. The static backend displays the plot directly in VS Code notebooks:
