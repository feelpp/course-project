...
NOTE: Advanced reference material. The course operational path is xref:m2:feelpp-gaya.adoc[Feel++ on Gaya with Slurm and Apptainer]. MPI launchers and libraries must match the host/container setup; Apptainer has no generic `exec --mpi` switch.
