...
NOTE: Advanced reference material. The course operational path is xref:m2:feelpp-gaya.adoc[{feelpp} on Gaya with Slurm and Apptainer]. MPI launchers and libraries must match the host/container setup; Apptainer has no generic `exec --mpi` switch.
...
Cemosis, the Center for Modeling and Simulation in Strasbourg, plays a critical role in advancing computational research through the development of {feelpp} within the {feelpp} Consortium. Here's why containerization matters for Cemosis:

* **Enhanced Reproducibility:**  
  - With {feelpp} being a sophisticated framework for finite element analysis, ensuring that every simulation runs in an identical environment is crucial for scientific validation. Containers guarantee that dependencies, libraries, and configurations remain consistent across all testing and production runs.

* **Simplified Collaboration:**  
  - The {feelpp} Consortium involves collaboration among multiple institutions. Containers enable researchers at Cemosis and partner organizations to share a single, standardized environment, reducing setup complexities and streamlining collaborative development.
...
* **Rapid Deployment and Testing:**  
  - With containerized workflows, Cemosis can quickly iterate on new features or bug fixes in {feelpp}, automatically testing changes in a controlled environment. This accelerates development cycles and improves the overall quality of the software.
...
=== {feelpp} Continuous Integration/Continuous Delivery (CI/CD)

Here is our strategy for CI/CD workflow

.{feelpp} CI/CD Workflow
image:feelpp/cicd/feelpp-cicd.png[title="{feelpp} CI/CD Workflow",width=75%]

=== {feelpp} Continous Benchmarking (CB)

Here is our strategy for CB workflow on EuroHPC and a few other systems

.{feelpp} CB Workflow
image:feelpp/cicd/feelpp-cd-hpc.png[title="{feelpp} EuroHPC CB",width=75%]
...
1. **Run** (on the HPC cluster, no root required):
[source,sh]
----
apptainer exec my_app.sif python my_script.py
----

1. **MPI Integration** (using native host libraries):
[source,sh]
----
mpirun -np 4 apptainer exec my_app.sif ./mpi_program
----
