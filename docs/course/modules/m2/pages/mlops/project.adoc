...
|===
a|
.3D tetrahedral mesh of the post and four fins
image::thermal-fin-mesh-3d.png[3D tetrahedral mesh of the central post and four thermal fins,width=100%]

Check the geometry, mesh quality and physical boundary markers before launching the parameter campaign. A valid mesh is required for every sampled pair latexmath:[(L,t)].

a|
.Nondimensional 3D temperature field
image::thermal-fin-temperature-3d.png[Nondimensional temperature field in the central post and four thermal fins,width=100%]

Heat enters through the CPU/root, diffuses through the post and fins, and is rejected to ambient air through the Robin boundaries. The full field supports verification; the boundary average latexmath:[S(\mu)] is the quantity learned by the surrogate.
|===
