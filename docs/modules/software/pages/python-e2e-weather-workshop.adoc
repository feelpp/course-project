...
dependencies = ["pandas>=2.0"] # Core runtime dependency for CSV loading and tabular analysis.

[project.optional-dependencies] # Optional dependency groups installed only when needed.
dev = ["pytest>=8.0", "ruff>=0.6", "build>=1.2"] # Test/lint/build tooling for local dev and CI.
plot = ["plotly>=5.20"] # Plot support for `weather-cli plot`.
dash = ["dash>=2.17", "dash-bootstrap-components>=1.6", "plotly>=5.20"] # Web dashboard stack.
...
== Dependency Map by Section

[cols="2,5",options="header"]
|===
|Workshop section |Python dependencies

|Project setup + package metadata
|`setuptools`, `wheel`, `pandas`

|CLI implementation
|`pandas` + Python standard library (`argparse`, `datetime`, `pathlib`)

|Testing and CI
|`pytest`, `ruff`, `build` (from `dev` extra)

|CLI plotting command (optional)
|`plotly` (from `plot` extra)

|Dash web app (optional)
|`dash`, `dash-bootstrap-components`, `plotly` (from `dash` extra)
|===
...
# add development and CI tooling
pip install -e ".[dev]"

# add dashboard dependencies when needed
...
Dependencies used in this section:

- Required: `pandas`
- Standard library: `argparse`, `datetime`, `pathlib`
- Optional (for `plot` subcommand): `plotly`
...
- `pytest`, `ruff`, `build` (install with `pip install -e ".[dev]"`)
- Keep Dash dependencies out of CI unless you test dashboard code in CI
...
For the weather project implementation details, use:
...
Install when starting this section:
...
- xref:visualisation:dash.adoc[Dash Tutorial (General Scientific Computing)]
