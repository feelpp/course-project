...
====
*🟡 LEVEL 2* | *Time:* 45 minutes | *Prerequisites:* Basic Docker commands
====
...
plt.title(f'∫sin(x)e^(-x)dx from 0 to π = {result:.4f}')
plt.xlabel('x')
plt.ylabel('f(x)')
...
1. xref:docker/hands-on/06-githubactions.adoc[Automate builds with GitHub Actions]
2. Learn xref:docker/hands-on/04-compose.adoc[Docker Compose] for multi-container setups
