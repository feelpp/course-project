...
====
*🟡 LEVEL 2* | *Time:* 20 minutes | *Prerequisites:* Basic Linux knowledge
====
...
With containers: docker pull myresearch/simulation:paper-2020 ✓
----
...
├── Python 3.11
├── NumPy 1.24.0 (requires specific BLAS)
├── SciPy 1.10.0 (must match NumPy version)
├── matplotlib 3.7.0
├── Your custom library
└── Specific compiler flags for performance
----
...
[source,text]
----
Layer 4: Your Python script (2 KB)        ← Changes frequently
Layer 3: pip install numpy scipy (200 MB) ← Changes sometimes  
Layer 2: Python 3.11 (100 MB)             ← Rarely changes
Layer 1: Ubuntu base (50 MB)              ← Almost never changes
----
...
. xref:docker-install.adoc[Install Docker] on your machine
. xref:hands-on/01-getting-started.adoc[Run your first container]
. Learn xref:dockerfile-basics.adoc[how to build images with Dockerfile]
